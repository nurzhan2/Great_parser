...
KEYWORDS: dict[str, list[str]] = {
    ...
    "услуги": ["ремонт", "услуги", "юрист", "образован", "курсы", "салон красоты",
               "клининг", "доставка", "набор на службу", "ваканс", "работа в",
               "мастер", "химчистка", "ателье"],
}

# Бренды, реально встреченные на щитах тестового обхода. Ключевые слова по
# смыслу («скидки», «квартиры») на щите обычно не написаны — написано название
# сети, поэтому без брендов классификация по тексту почти всегда молчит.
BRANDS: dict[str, list[str]] = {
    "ретейл": ["wildberries", "wb.ru", "ozon", "магнит", "пятёрочка", "пятерочка",
               "перекрёсток", "перекресток", "дикси", "лента", "ашан", "метро",
               "kfc", "rostic", "ростикс", "макдоналдс", "вкусно и точка", "бургер",
               "л'этуаль", "летуаль", "sunlight", "санлайт", "улыбка радуги",
               "miele", "bosch", "самокат", "вкусвилл", "фикс прайс", "fix price",
               "спортмастер", "детский мир", "эльдорадо", "днс", "dns", "мвидео",
               "золото", "ювелир", "шаурма", "пекарня", "кофе", "пассаж", "молл"],
    "авто": ["лукойл", "газпромнефть", "роснефть", "татнефть", "shell", "азс",
             "дизельных", "двигател", "автозапчаст", "колёса", "колеса"],
    "финансы": ["сбер", "втб", "тинькофф", "т-банк", "альфа-банк", "газпромбанк",
                "открытие", "совкомбанк", "кэшбэк", "кешбэк", "выплата",
                "погашение ипотеки", "вклад под"],
    "развлечения": ["киномакс", "кинопоказ", "формула кино", "каро", "спасская башня",
                    "выставка", "фестивал", "день индии", "лето в москве",
                    "музей", "цирк", "аквапарк", "мультимедийная"],
    "медицина": ["инвитро", "гемотест", "медси", "он клиник", "стоматологи",
                 "оптика", "зрение", "линзы"],
    "недвижимость": ["самолет", "самолёт", "пик", "а101", "донстрой", "эталон",
                     "гранель", "мр групп", "офис продаж", "квартал", "жк "],
}
...
class KeywordClassifier(Classifier):
    ...
    def classify(self, text: str, image: Optional[Image.Image] = None) -> str:
        ...
        # Бренд весит больше тематического слова: на щите пишут название сети,
        # а не «скидки». Совпадение бренда сразу задаёт тему.
        best, best_hits = "другое", 0
        for cat, words in BRANDS.items():
            hits = 2 * sum(1 for w in words if w in low)
            if hits > best_hits:
